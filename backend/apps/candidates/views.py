...
# ---------------------------------------------------------------------------
# Shared mixin - eliminates repeated profile lookup across all candidate views
# ---------------------------------------------------------------------------

class _CandidateProfileMixin:
    """Resolves the current user's CandidateProfile once, reused by all sub-entity views."""
    ...
    def _get_child_queryset(self, model_class):
        profile = self._get_profile()
        return model_class.objects.filter(candidate=profile) if profile else model_class.objects.none()
...
class CandidateProfileView(_CandidateProfileMixin, generics.RetrieveUpdateAPIView):
    """Get or update the current candidate's profile."""
    serializer_class = CandidateProfileSerializer
    permission_classes = [IsAuthenticated, IsCandidate]

    def get_object(self):
        return self._get_profile(create=True)

    def perform_update(self, serializer):
        serializer.save()


class CandidateResumeUploadView(_CandidateProfileMixin, generics.UpdateAPIView):
    """Upload resume PDF."""
    serializer_class = CandidateProfileSerializer
    permission_classes = [IsAuthenticated, IsCandidate]
...
class CandidateBannerUploadView(_CandidateProfileMixin, generics.UpdateAPIView):
    """Upload banner image."""
    serializer_class = CandidateProfileSerializer
    permission_classes = [IsAuthenticated, IsCandidate]
    parser_classes = [MultiPartParser, FormParser]
...
class CandidateSkillListCreateView(_CandidateProfileMixin, generics.ListCreateAPIView):
    serializer_class = CandidateSkillSerializer
    permission_classes = [IsAuthenticated, IsCandidate]

    def get_queryset(self):
        return self._get_child_queryset(CandidateSkill)

    def perform_create(self, serializer):
        serializer.save(candidate=self._get_profile(create=True))


class CandidateSkillDeleteView(_CandidateProfileMixin, generics.DestroyAPIView):
    serializer_class = CandidateSkillSerializer
    permission_classes = [IsAuthenticated, IsCandidate]
...
class ExperienceListCreateView(_CandidateProfileMixin, generics.ListCreateAPIView):
    serializer_class = ExperienceSerializer
    permission_classes = [IsAuthenticated, IsCandidate]

    def get_queryset(self):
        return self._get_child_queryset(Experience)

    def perform_create(self, serializer):
        serializer.save(candidate=self._get_profile(create=True))


class ExperienceDetailView(_CandidateProfileMixin, generics.RetrieveUpdateDestroyAPIView):
    serializer_class = ExperienceSerializer
    permission_classes = [IsAuthenticated, IsCandidate]
...
class EducationListCreateView(_CandidateProfileMixin, generics.ListCreateAPIView):
    serializer_class = EducationSerializer
    permission_classes = [IsAuthenticated, IsCandidate]

    def get_queryset(self):
        return self._get_child_queryset(Education)

    def perform_create(self, serializer):
        serializer.save(candidate=self._get_profile(create=True))


class EducationDetailView(_CandidateProfileMixin, generics.RetrieveUpdateDestroyAPIView):
    serializer_class = EducationSerializer
    permission_classes = [IsAuthenticated, IsCandidate]
...
class CertificationListCreateView(_CandidateProfileMixin, generics.ListCreateAPIView):
    serializer_class = CertificationSerializer
    permission_classes = [IsAuthenticated, IsCandidate]

    def get_queryset(self):
        return self._get_child_queryset(Certification)

    def perform_create(self, serializer):
        serializer.save(candidate=self._get_profile(create=True))


class CertificationDetailView(_CandidateProfileMixin, generics.RetrieveUpdateDestroyAPIView):
    serializer_class = CertificationSerializer
    permission_classes = [IsAuthenticated, IsCandidate]
...
class CandidateRecommendedView(CandidateTopView):
    """Placeholder: mirrors TopView, expandable to filter by recruiter's industry/company."""
    pass
