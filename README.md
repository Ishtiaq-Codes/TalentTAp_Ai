...
### For Candidates
* **Rich Profiles:** Build professional, comprehensive profiles with dynamic completion tracking and portfolio integration.
* **Smart Applications:** Seamlessly apply for jobs that match your skills based on AI semantic matching.
* **AI Video Interviews:** Undertake automated, conversational AI interviews directly in the browser with real-time feedback.
* **Real-time Tracking:** Monitor application statuses via an interactive candidate dashboard.

### For Recruiters
* **Copilot Assistant:** A dedicated AI assistant to help draft outreach messages, analyze candidate gaps, and generate interview guides.
...
* **Talent Pipelines:** Manage candidates through customizable Kanban-style pipelines (Applied -> Reviewing -> Shortlisted -> Interview -> Offered).

### For Companies
* **Command Center Dashboard:** A high-level overview of hiring metrics, recruiter performance, and active job postings.
* **Team Management:** Add and manage multiple recruiters under a single company umbrella with Role-Based Access Control (RBAC).
* **Talent Pools:** Save and categorize high-potential candidates across the entire organization for future opportunities.
* **Premium Subscriptions:** Unlock advanced features and unlimited job postings via seamless Stripe billing integration.

### Platform Security & Anti-Cheat
* **MediaPipe Facial Tracking:** The AI Interview room utilizes real-time facial landmark detection to prevent cheating (e.g., looking away, multiple faces, out of frame).
...
You will need to configure your environment variables. Create a `.env` file in the `backend` directory with your database credentials, Django Secret Key, Groq API Key, and Stripe API keys.
